...
def load_rep_file_list(rep_file, return_timepoints=False):
    """
    :param rep_file: Lists which files in its directory to load
    :return: dfs, genes, geneTS, df, timekeys, num_per_keys, timepoints
    ...
    print "Timkeyes are ", timekeys
    df = avg_and_std(df, keys=timekeys)

    if return_timepoints:
        timepoints = [timekey_to_timepoint(timekey) for timekey in timekeys]
        return dfs, genes, geneTS, df, timekeys, num_per_keys, timepoints
    else:
...
def load_params(filename):
    """
    Load params of csv file where first value is key, second is value
    :param filename:
    :return: param_dict
    """
...
def timekey_to_timepoint(timekey):
    """
    Return the timepoint it corresponds to. Assumes are of form "xxxxt00_rep..."
    """

    timestring = timekey.split("_")[0].split("t")[-1]
    if timestring == "05":
        return 0.5
    else:
        return eval(timestring)
...
if HAS_PLT:
    def plot_genes(data, genes, gene_col="gene", keys = timekeys, title="Gene Expression From a few species",
                   ...
                   horizontal_line_color_labels=None, horizontal_line_style="solid", xlim=None, ylim=None, filename=None, plot_bar=True, plot_reps=False, repkeys=repkeys,
                   gene_labels=None,
                   plot_outside=False,
                   verbose=False,
        ...
        for gene, i in zip(genes, range(len(genes))):
            ...
            if plot_bar:
                try:
                    ...
                    (_, caps, eb) = plt.errorbar(timepoints, gene_avg,
                    ...
                    plt.errorbar(timepoints, gene_avg, label=label,
                                 linewidth=linewidth)
            else:
                plt.errorbar(timepoints, gene_avg, label=label,
                                 linewidth=linewidth)
        ...
        if horizontal_line_color_labels !=None:
            for line, color, label in horizontal_line_color_labels:
                plt.axhline(line, color=color,label=label, linestyle=horizontal_line_style)

        if xlim != None:
            plt.xlim(xlim[0], xlim[1])
        else:
            plt.xlim(timepoints[0] - 1, timepoints[-1] + 1)
        if ylim != None:
